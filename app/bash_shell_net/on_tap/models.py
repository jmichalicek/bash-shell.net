...
class BatchOnTapRecord(models.Model):
    ...
    on_tap_date = models.DateField(null=True, blank=True)
    off_tap_date = models.DateField(null=True, blank=True)
    never_on_tap = models.BooleanField(blank=True, default=False)

    class Meta:
        indexes = [models.Index(fields=["on_tap_date", "off_tap_date"])]
        ordering = ("-on_tap_date", "-off_tap_date")

    def __str__(self) -> str:
        val = f"{self.batch_log_page} from {self.on_tap_date}"
...
class OnTapPage(Page):  # type: ignore
    ...
    def get_on_tap_batches(self: "OnTapPage") -> "QuerySet[BatchOnTapRecord]":
        ...
        """
        return (
            BatchOnTapRecord.objects.filter(off_tap_date=None, on_tap_date__lte=timezone.now().date())
            .filter(batch_log_page__in=BatchLogPage.objects.descendant_of(self).live())
            .select_related("batch_log_page__recipe_page__style")
    ...
    def get_past_batches(self) -> "QuerySet[BatchOnTapRecord]":
        ...
        """
        return (
            BatchOnTapRecord.objects.filter(
                Q(off_tap_date__lte=timezone.now().date()) | Q(never_on_tap=True),
                batch_log_page__in=BatchLogPage.objects.descendant_of(self).live(),
            )
            .select_related("batch_log_page__recipe_page__style")
            .order_by("-batch_log_page__brewed_date", "-on_tap_date")
        )
